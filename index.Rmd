...
First, load up the CSV I scraped.

```{r getdata}
d.data <- read_csv(
  "run_results.csv", 
  col_types = cols(
    Wine_price = col_number()
    )
  )
```

Now, extract year of production from the name. This is rough...
...
Create some dummy variables to define the wine.

```{r wintypes}

d.grepme <- data.frame(
  variablename = c(
    "white","sparkling","red","eiswein","trocken",
    "spatlese","qba","mosel","rheingau","pfalz",
    "franken","nahe","baden","rheinhessen","wurttemberg"
  ),
  searchstatement = c(
    "riesling|müller-thurgau|pinot gris|grauburgunder|weissburgunder|pinot blanc|gewürztraminer|silvaner|gutedel|chasselas|chardonnay|kerner|scheurebe|white",
    "sparkling","red|pinot noir|spätburgunder",
    "eiswein|ice wine","trocken|dry","spätlese",
    "qba","mosel|mosel-saar-ruwer","rheingau",
    "pfalz","franken","nahe","baden","rheinhessen",
    "württemberg"
  ),
  stringsAsFactors = F
)

for(i in 1:nrow(d.grepme)){
  d.data[,d.grepme[i,"variablename"]] <- 0

  d.data[
    grep(
      d.grepme[i,"searchstatement"],
      tolower(d.data$Wine_name)
    ),
    d.grepme[i,"variablename"]
  ] <- 1 
}
```
